// получить елемент по ID
	...
	async function getData(address){
		...
		//console.log(xls)
		const json = await XLS.utils.sheet_to_json(xls.Sheets['Лист1']);
		return json
	}
	//загружаем данные из спецификации и отпраляем их в переменную objs
	getData(url).then(json => objs = json)
		...
		let t = el("mod_sel").options[el("mod_sel").selectedIndex].text;
			...
			for( obj in objs){
				if(objs[obj]['Модель']===t){
					...
					for(key in objs[obj]){
						if(key === "Описание"){
								el("devices").append('Вы выбрали: '+objs[obj][key].toString().toLowerCase()+' '+objs[obj]['Модель']);
							}
						if(key === "Мощность, Вт"){
								let st = objs[obj][key];
								...
								for(let i=0; i<arr.length; i++){
									str = str + '<option value = "p' + i +'">'+arr[i]+'</option>'
								}
								str =str +'</select>'
